"""
Hyperparameter Optimization with FIXED SPLIT (seed 314)

New baseline: 86.61% F1 (reproducible, stratified)
Target: 92-95% F1
Improvement needed: +5.4 to +8.4 points
"""
...
class ConfigurableGAT(torch.nn.Module):
    """Flexible GAT supporting both GAT and GATv2"""
...
def train_and_evaluate(trial, data, device, trial_num, total_trials):
    ...
    # EXPANDED search space to reach 92%+
    hidden_dim = trial.suggest_categorical('hidden_dim', [256, 320, 384, 448, 512])
    num_heads = trial.suggest_categorical('num_heads', [6, 8, 10, 12])
...
def main():
    ...
    print("  hidden_dim: [256, 320, 384, 448, 512]")
    print("  num_heads: [6, 8, 10, 12]")
    print("  num_layers: [3, 4, 5]")
    print("  dropout: [0.2, 0.25, 0.3, 0.35, 0.4]")
    print("  lr: [0.0005, 0.003] (log scale)")
    ...
    print(f"Target: 92-95% F1 (+5.4 to +8.4 points)\n")
    ...
    if test_f1 >= target_f1:
        ...
        progress_pct = (improvement / (target_f1 - baseline_f1)) * 100 if improvement > 0 else 0
        print(f"\nRemaining gap: {gap:.4f} ({gap*100:.2f} points)")
        if progress_pct > 0:
            print(f"Progress:      {progress_pct:.1f}% of the way to target")
