"""
Train ensemble with Node2Vec embeddings (522 features) - FIXED VERSION
Uses exact same architecture as successful single model (91.13% F1)
Expected: 91.5-92.0% F1 
"""
...
print("=" * 80)
print("NODE2VEC ENSEMBLE TRAINING (FIXED)")
print("=" * 80)
print()
...
# Model definitions (EXACT same as working single model)
class GATv2Model(nn.Module):
    """GATv2 - exact architecture that achieved 91.13% F1"""
...
def evaluate(model, data, mask):
    ...
    f1 = f1_score(y_true, y_pred, average='weighted')  # Use weighted like successful models
    acc = accuracy_score(y_true, y_pred)
    
    # Get probabilities for ensemble
    probs = F.softmax(out[mask], dim=1).cpu()
    
    return f1, acc, probs

def train_model(model_name, model, data, train_mask, val_mask, lr, weight_decay, seed, epochs=200, patience=20):
...
# Train 3 models with different seeds (EXACT same hyperparameters as working model)
print("=" * 80)
print("TRAINING INDIVIDUAL MODELS")
print("=" * 80)

models_info = [
    ('GATv2-42', GATv2Model, 42, 0.0016, 0.000134),
    ('GATv2-314', GATv2Model, 314, 0.0016, 0.000134),
    ('GIN-999', GINModel, 999, 0.001, 5e-4),
]

trained_models = []
val_f1s = []

for model_name, ModelClass, seed, lr, weight_decay in models_info:
    ...
    model, val_f1 = train_model(model_name, model, data, train_mask, val_mask, 
...
for (model_name, model, seed), val_f1 in zip(trained_models, val_f1s):
    ...
    print(f"  Class 0 recall: {class_0_recall:.4f} (baseline: 0.7303)")
...
# Calculate metrics
ensemble_f1 = f1_score(y_true, ensemble_pred, average='weighted')  # Use weighted like successful models
...
# Save results
output_dir = Path('experiments/node2vec_ensemble_fixed')
